'use strict';

const createError = require('http-errors');
const firebase = require('firebase');
const admin = require('firebase-admin');

module.exports = (req, res, next) => {
  if (!req.headers.authorization)
    return next(createError(401, 'No Authorization header provided'));

  let encoded = req.headers.authorization.split('Basic ')[1];
  if(!encoded)
    return next(createError(401, 'No Basic Authorization header provided'));

  let decoded = new Buffer(encoded, 'base64').toString();
  if(!decoded)
    return next(createError(401, 'no base64 String given'));

  let [email, password] = decoded.split(':');
  if(!email || !password)
    return next(createError(401, 'There was no email or password given'));

  firebase.auth().signInWithEmailAndPassword(email, password)
  .then(user => {
    return admin.auth().createCustomToken(user.uid);
  })
  .then(token => {
    req.token = token;
    return firebase.auth().signOut();
  })
  .then(() => next())
  .catch(err => next(createError(401, err.message)));
};
